...
fusionSites =
  ...
  , "    add_header Content-Security-Policy-Report-Only \"img-src 'self'; style-src 'self'; connect-src 'self'; script-src 'self' 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'; report-uri https://fusionapp.report-uri.com/r/d/csp/wizard\";"
  , ""
  , "    location /fusion-error {"
  , "        root            /srv/nginx;"
  , "    }"
